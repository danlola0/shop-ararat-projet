...
import MouvementSimpleForm from './MouvementSimpleForm';
...
const categories = [
  {
    ...
    forms: [
      ...
      { id: 'mouvement', label: 'Mouvements', component: MouvementSimpleForm }
  ...
  return (
    <div className="p-2 sm:p-6 w-full">
      ...
      <div className="border-b border-gray-200 mb-4 sm:mb-6 overflow-x-auto w-full">
        <nav className="-mb-px flex space-x-2 sm:space-x-8 w-full">
          {categories.map((cat) => (
            <button
              key={cat.id}
              onClick={() => {
      ...
      <div className="border-b border-gray-100 mb-4 flex space-x-1 sm:space-x-4 w-full overflow-x-auto">
        {currentCategory.forms.map((form) => (
          <button
            key={form.id}
            onClick={() => setActiveForm(form.id)}
            className={`py-1 px-2 border-b-2 font-medium text-xs sm:text-sm transition-colors whitespace-nowrap ${
              activeForm === form.id
      ...
      {/* Formulaire actif */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-2 sm:p-6 w-full">
        <ActiveComponent />
      </div>
    </div>
  );
